...
class CyberGoose:
    ...
    def _draw_character(self, step=0):
        ...
        c.create_text(70, 116, text=texts[(step // 15) % 4],
        ...
        # Chaussures
        c.create_rectangle(28, 172+lo,  66, 181+lo,  fill=GREEN, outline="")
        c.create_rectangle(74, 172-lo, 112, 181-lo, fill=GREEN, outline="")
        # Badge bas
    ...
    def _move_loop(self):
        ...
        while True:
            if self.follow_mouse:
                ...
                self.dx = max(-14, min(14, diff * 0.18))
                target_y = my - WIN_H
                self.y  += (target_y - self.y) * 0.09
            else:
                self.x += self.dx
                if self.x <= 0 or self.x >= self.sw - WIN_W:
                    self.dx *= -1
                self.y = (self.sh - WIN_H - 60) + 18 * (
    ...
    # ── Bulles conseils (plus fréquentes) ───────────────────────────────
    ...
    # ── Trolling souris (plus violent) ──────────────────────────────────
    def _mouse_troll_loop(self):
        time.sleep(15)
        while True:
            time.sleep(random.randint(12, 24))
            action = random.choice(["nudge", "circle", "follow", "lock"])

            if action == "nudge":
                for _ in range(random.randint(20, 40)):
                    cx, cy = _get_cursor()
                    _set_cursor(cx + random.randint(-35, 35),
                                cy + random.randint(-35, 35))
                    time.sleep(0.03)
                ...
                for i in range(96):
                    ...
                    _set_cursor(cx + int(90 * math.cos(angle)),
                                cy + int(90 * math.sin(angle)))
                    time.sleep(0.015)
                _set_cursor(cx, cy)
                ...
                # "Colle" la souris au centre de l'écran quelques secondes
                center_x = self.sw // 2
                center_y = self.sh // 2
                for _ in range(80):
                    _set_cursor(center_x + random.randint(-5, 5),
    ...
    def _spawn_alert(self):
        ...
        ax = random.randint(20, max(30, self.sw - 320))
        ay = random.randint(20, max(30, self.sh - 200))
        aw.geometry(f"+{ax}+{ay}")
        _beep(1200, 200)
    ...
    def _spawn_terminal(self):
        ...
        tx = random.randint(20, max(20, self.sw - 460))
        ty = random.randint(20, max(20, self.sh - 280))
        tw.geometry(f"+{tx}+{ty}")

        def type_line(idx=0):
            if idx >= len(TERMINAL_LINES):
                threading.Timer(10, lambda: self.root.after(0, self._safe_destroy, tw)).start()
                return
            ...
            _beep(random.randint(250, 900), 30)
            tw.after(random.randint(140, 380), type_line, idx + 1)

        tw.after(200, type_line)

    # ── Clones mini (plus souvent) ──────────────────────────────────────
    ...
    def _spawn_clone(self):
        ...
        clone.wm_attributes("-alpha", 0.9)

        cx = random.randint(30, self.sw - cw - 30)
        cy = random.randint(30, self.sh - ch - 30)
        ...
        def draw_mini(step=0):
            ...
            cv.after(90, draw_mini, step + 1)

        draw_mini()

        speed = [random.choice([-3, 3]), random.choice([-2, 2])]
        ...
        move()
        threading.Timer(40, lambda: self.root.after(0, self._safe_destroy, clone)).start()

    # ── Overlay plein écran "prise d'otage" ─────────────────────────────
    def _overlay_loop(self):
        time.sleep(20)
        while True:
            time.sleep(random.randint(20, 45))
            self.root.after(0, self._spawn_overlay)

    def _spawn_overlay(self):
        # Si déjà un overlay, on le remplace
        self._safe_destroy(self.overlay_win)
        ow = tk.Toplevel(self.root)
        self.overlay_win = ow
        ow.overrideredirect(True)
        ow.attributes("-topmost", True)
        ow.wm_attributes("-alpha", 0.88)
        ow.configure(bg="#000000")

        ow.geometry(f"{self.sw}x{self.sh}+0+0")

        msg1 = tk.Label(
            ow,
            text="⚠ TON PC N'EST PAS PROTÉGÉ ⚠",
            bg="#000000", fg=RED,
        ...
        msg2 = tk.Label(
            ow,
            text="Un simple périphérique USB peut exécuter du code\n"
                 "sans que tu ne cliques sur quoi que ce soit.",
            bg="#000000", fg=GREEN,
            font=("Courier", 14),
            justify="center"
        )
        ...
        msg3 = tk.Label(
            ow,
            text="[ DEMO PÉDAGOGIQUE ]  Ctrl+Shift+Q pour arrêter",
            bg="#000000", fg="#888888",
            font=("Courier", 11)
        )
        msg3.pack(pady=40)
        ...
        # Effet de flash
        def flash(step=0):
            if not ow.winfo_exists():
                return
            col = "#220000" if step % 2 == 0 else "#000000"
            ow.configure(bg=col)
            msg1.configure(bg=col)
            msg2.configure(bg=col)
            msg3.configure(bg=col)
            if step < 30:
                ow.after(150, flash, step + 1)

        flash()
        _beep(1000, 200)
        _beep(600, 200)
        # Retire l’overlay après ~12 s
        threading.Timer(12, lambda: self.root.after(0, self._safe_destroy, ow)).start()
